...
export default function ExpiryRescuePage() {
 ...
 case "CRITICAL_30":
 return (
 <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-black bg-white text-rose-600 border border-rose-600 animate-pulse">
 ⚡ {days}d to expiry (≤30d)
 </span>
 );
 case "WARNING_60":
 return (
 <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-bold bg-white text-amber-600 border border-amber-600">
 ⏳ {days}d to expiry (31-60d)
 </span>
 );
 case "ATTENTION_90":
 return (
 <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-white text-blue-600 border border-blue-600">
 🕒 {days}d to expiry (61-90d)
 </span>
 );
 default:
 return (
 <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-slate-100 text-slate-500 border border-slate-200">
 ✓ {days}d to expiry
 </span>
 );
 }
 ...
 switch (priority) {
 case "HIGH":
 return <span className="px-2.5 py-1 rounded-full text-xs font-black bg-white text-purple-600 border border-purple-600">🔥 HIGH PRIORITY RESCUE</span>;
 case "MEDIUM":
 return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-white text-amber-600 border border-amber-600">⚡ MEDIUM PRIORITY</span>;
 default:
 return <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-white text-blue-600 border border-blue-600">ℹ️ ROUTINE RESCUE</span>;
 }
 };
 ...
 <p className="mt-1 text-[11px] text-slate-500">Monitored in Grid</p>
 </div>
 ...
 <div
 key={opp.batch_id}
 className="p-5 rounded-2xl border border-slate-200 bg-white hover:border-purple-600/80 transition-all shadow-xs"
 >
 <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
 <div>
 <div className="flex items-center gap-2.5 flex-wrap">
 <h3 className="text-lg font-bold text-slate-900">{opp.medicine_name}</h3>
 <span className="text-xs font-mono text-purple-600 bg-white px-2 py-0.5 rounded border border-slate-200">
 Batch #{opp.batch_number}
 </span>
 {getPriorityBadge(opp.priority)}
 </div>
 <p className="text-xs text-slate-500 mt-1">
 📍 Source Facility: <strong className="text-slate-700">{opp.source_facility_name || opp.source_warehouse_name}</strong> • Category: {opp.category}
 </p>
 </div>

 <div className="text-left md:text-right">
 <div className="text-xs text-slate-500">Rescueable Surplus</div>
 ...
 <span className="text-slate-500 block mb-0.5">Expiry Date & Timeline</span>
 <strong className="text-amber-600 font-mono">{opp.expiry_date} ({opp.days_until_expiry} days remaining)</strong>
 </div>

 <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
 <span className="text-slate-500 block mb-0.5">Expected Local Consumption</span>
 <strong className="text-slate-700 font-mono">{opp.expected_local_consumption} units before expiry</strong>
 </div>

 <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
 <span className="text-slate-500 block mb-0.5">FEFO Intelligence Reason</span>
 <span className="text-purple-600 leading-snug">{opp.reason}</span>
 </div>
 </div>
 </div>
 ))
 ...
 b.is_rescue_candidate ? "bg-white" : ""
 }`}
 >
 ...
 <td className="px-6 py-4">
 <div className="font-mono text-slate-700 font-semibold">{b.expiry_date}</div>
