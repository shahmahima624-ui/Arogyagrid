...
const STATUS_BADGES: Record<TransferStatus, { label: string; color: string; icon: any }> = {
 PENDING: { label: "Pending Approval", color: "bg-white text-amber-600 border-amber-700", icon: Clock },
 APPROVED: { label: "Approved for Transit", color: "bg-white text-blue-600 border-blue-700", icon: CheckCircle2 },
 IN_TRANSIT: { label: "In Transit 🚚", color: "bg-white text-purple-600 border-purple-700 animate-pulse", icon: Truck },
 RECEIVED: { label: "Received & Reconciled", color: "bg-white text-emerald-600 border-emerald-700", icon: PackageCheck },
 REJECTED: { label: "Rejected", color: "bg-white text-rose-600 border-rose-700", icon: XCircle },
...
function StatusStepper({ status }: { status: TransferStatus }) {
 ...
 return (
 <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-white text-rose-600 border border-slate-200">
...
export default function StockTransfersPage() {
 ...
 <div className="rounded-xl bg-white border border-emerald-700 px-4 py-2.5 text-xs text-emerald-600 font-bold flex items-center gap-2">
 ...
 <div className="text-xs font-bold text-amber-600 uppercase tracking-wider">Pending Approval</div>
 <div className="text-3xl font-black text-amber-600 mt-1">{loading ? "..." : pendingCount}</div>
 <div className="text-[11px] text-slate-500 font-medium mt-0.5">Awaiting admin review</div>
 </div>

 <div className="rounded-2xl border border-slate-200 bg-white p-4 ">
 <div className="text-xs font-bold text-purple-600 uppercase tracking-wider">In Transit 🚚</div>
 <div className="text-3xl font-black text-purple-600 mt-1">{loading ? "..." : inTransitCount}</div>
 <div className="text-[11px] text-slate-500 font-medium mt-0.5">Stock en route</div>
 </div>

 <div className="rounded-2xl border border-slate-200 bg-white p-4 ">
 <div className="text-xs font-bold text-emerald-600 uppercase tracking-wider">Completed Transfers</div>
 <div className="text-3xl font-black text-emerald-600 mt-1">{loading ? "..." : receivedCount}</div>
 <div className="text-[11px] text-slate-500 font-medium mt-0.5">Inventory reconciled</div>
 </div>
 ...
 <div
 key={t.id}
 className="p-5 rounded-2xl border border-slate-200 bg-white hover:border-slate-200 transition-all shadow-xs"
 >
 <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
 {/* Left: Info */}
 <div>
 <div className="flex items-center gap-3 flex-wrap">
 <span className="font-mono text-xs font-black text-slate-600 bg-white px-2.5 py-1 rounded-md border border-slate-200">
 {t.tracking_number}
 </span>
 ...
 <span className="text-purple-600 font-semibold">{t.medicine_name}</span>
 <span>•</span>
 ...
 disabled={!!actionLoading}
 className="px-3 py-2 rounded-xl bg-white hover:bg-white border border-rose-700 text-xs font-bold text-rose-600 transition-all"
 >
 Reject
 </button>
 </>
