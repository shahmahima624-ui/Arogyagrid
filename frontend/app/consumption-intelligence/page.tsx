      ...
      {data && <>
        ...
        <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-6 flex h-56 items-end gap-1 border-b border-slate-200 pb-1">
            {chart.map((point) => <div key={point.date} className="group relative flex h-full flex-1 items-end"><div title={`${point.date}: ${point.quantity_consumed} units`} className="w-full rounded-t bg-emerald-500 transition hover:bg-emerald-700" style={{ height: `${Math.max(2, (point.quantity_consumed / chartMax) * 100)}%` }} /><span className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded bg-slate-900 px-2 py-1 text-xs text-white group-hover:block">{point.date}: {point.quantity_consumed}</span></div>)}
          </div>
          <div className="mt-2 flex justify-between text-xs text-slate-400"><span>{chart[0]?.date}</span><span>{chart.at(-1)?.date}</span></div>
        </section>
